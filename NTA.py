...
packets = rdpcap('/content/20160423_235403.pcap')
...
def analyze_packet_data(df):
    ...
    # Calculate percentage within each group and reset index to align with the DataFrame
    ip_communication_protocols["Percentage"] = ip_communication_protocols.groupby(["Source IP", "Destination IP"])["Count"].apply(lambda x: x / x.sum() * 100).reset_index(level=[0,1], drop=True)
...
if __name__ == "__main__":
    pcap_file = '/content/20160423_235403.pcap'
